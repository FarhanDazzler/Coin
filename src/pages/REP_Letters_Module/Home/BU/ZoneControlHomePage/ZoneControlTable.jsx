...
const ZoneControlTable = ({ zoneValue, setZoneValue, buValue, setBUValue }) => {
  ...
  useEffect(() => {
    if (yearValue.length > 0) {
      ...
      dispatch(get_BU_Zone_ControlHomePageData(payload));
    } else {
      toast.error('Please select Year in filter.');
    }
  }, [
      ...
      Cell: (row) => {
        return (
          <span className={'text-yellow-dark'}>
            {row.row.original?.signatures?.zc_signed === false ? 'Approval Pending' : 'Signed'}
          </span>
        );
      },
    },
...
export default ZoneControlTable;
