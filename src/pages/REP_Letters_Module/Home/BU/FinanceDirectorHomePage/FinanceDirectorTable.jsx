...
import {
  get_BU_Finance_DirectorHomePageDataSelector,
...
const FinanceDirectorTable = ({ zoneValue, setZoneValue, buValue, setBUValue }) => {
  ...
  useEffect(() => {
    if (yearValue.length > 0) {
      ...
      dispatch(get_BU_Finance_DirectorHomePageData(payload));
    } else {
      toast.error('Please select Year in filter.');
    }
  }, [
  ...
  const HomePageData = useMemo(() => {
    return getHomePageData?.data[0]?.financeDirectorData || [];
          ...
          <div>
            ...
            {['Pending RBA Approval'].includes(row.row.original.RBA_Status) && (
              <Button
                ...
                onClick={() => {
                  history.push(
                    `/REP-Letters/attempt-letter/BU-letter-form/${row.row.original.id}/approveSection3`,
                  );
                }}
              >
                Approve RBA
      ...
      Cell: (row) => {
        return (
          <span className={'text-yellow-dark'}>
            {row.row.original?.signatures?.fd_signed === false ? 'Approval Pending' : 'Signed'}
          </span>
        );
      },
    },
...
export default FinanceDirectorTable;
